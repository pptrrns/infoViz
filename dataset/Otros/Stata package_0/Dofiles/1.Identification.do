	...
	* Official WIID
	...
	use "$data" , clear
	* Tagging selected observations, series type
	merge 1:1 id using seriestype , keepus (giniseries shareseries)
	
	* In any case, check LIS observations (the others are connected by id in older official dataset, here that id has not changed)
	replace giniseries  = 1 if country=="China" 	   & source==1 & wiidc==1 & year==2014
	replace shareseries = 1 if country=="China" 	   & source==1 & wiidc==1
	replace giniseries  = 1 if country=="South Africa" & source==1 & wiidc==1
	replace shareseries = 1 if country=="South Africa" & source==1 & wiidc==1
	
	recode shareseries giniseries (.=0)
	ta _merge wiidc , miss
	drop _merge
	...
	* Linking some series based on source, with no adjustment since there is no overlapping
	...
	replace idsource = 73 if idsource==70 & source_d != "PovcalNet"	
	...
	lab def idsource 10 LIS 11 "ERF/LIS" 20 Eurostat 21 ECHP 22 EUSILC 30 "SEDLAC" 40 UN 41 "Altimir 1986" 42 "ECLAC" 43 "ECLAC 2016" 44 "ECLAC" 45 UNICEF 50 NSA 60 OECD 70 "WB 2020" 71 "Deininger and Squire 2004" 72 "Jain 1975" 73 "WB other" 80 Research 90 "IADB" 91 "Dowling and Soo 1983" 92 "ADB, other" 93 ILO 94 IMF , modify
...
*	di "series = `a'`y'`z' "
	...
	*****************************************************************************************
	...
	gen year0c = year
	gen year1c = year
	collapse (min) nobs idcountry c3 c2 region_wb incomegroup year0c nyearsc nseries nsource nsourced nidsource (max) year1c , by(country)
	...
	* series
	preserve
	collapse (min) nobs idcountry c3 c2 region_wb incomegroup former histent year0 nyears nseries idseries index idsource* resource scale source_d nsource nsourced nidsource (max) year1 (mean) wiidcseries shareseriesc giniseriesc , by(country series)
	...
	* Resources
	...
	collapse (sum) one  (min) nobs idcountry c3 c2 region_wb incomegroup year0 nyears nseries nsource nsourced nidsource (max) year1 , by(country resourceb)
