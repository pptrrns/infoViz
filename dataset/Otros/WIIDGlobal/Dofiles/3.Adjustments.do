	...
	* Code that executes Phase 1 (Integration), Part 1 (gini and p1-p50)
	...
	* ADO file that will be used to compute Gini
	do "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Dofiles\ineqwiid"
	...
	* India: integraring D&S into WB series /* only Gini, D&S2004, shares are inconsistent */
	...
	replace source_d="Poverty and Inequality Platform (PIP)"				if source_d=="Deininger and Squire, World Bank 2004" & country=="India"
	...
	replace source_d="Poverty and Inequality Platform (PIP)" if source_d=="PovcalNet 2019" /*& country=="Ecuador"*/
	...
	replace source_d="Poverty and Inequality Platform (PIP)" 	   if source_d=="van Ginneken and Park 1984" & country=="Egypt"
	sort country source_d series
	by country source_d: replace   series =   series[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="Egypt"
	by country source_d: replace idseries = idseries[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="Egypt"
	
	replace source_d="Poverty and Inequality Platform (PIP)" if source_d=="World Bank 2002" & country=="Jamaica"
	replace source_d="Poverty and Inequality Platform (PIP)" if source_d=="World Bank 2002" & country=="China"
	replace source_d="Poverty and Inequality Platform (PIP)" if source_d=="PovcalNet 2021" & country=="South Africa"

	sort country source_d 	
	
	by country source_d : replace idseries = idseries[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="Jamaica"
	by country source_d : replace   series =   series[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="Jamaica"
	by country source_d : replace idseries = idseries[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="China"
	by country source_d : replace   series =   series[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="China"
	by country source_d : replace idseries = idseries[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="South Africa"
	by country source_d : replace   series =   series[_N] if source_d=="Poverty and Inequality Platform (PIP)" & country=="South Africa"
	...
	* Changing year (to improve connection among series)
	...
	replace year = year + 1 if source_d=="Poverty and Inequality Platform (PIP)"  & country=="Kosovo" 	 & year==2017
	replace year = year + 1 if source==5 						 & country=="Canada" 
	
	replace year = year - 1 if source==6 						 & country=="Japan"		 & year>2009 
	
	replace year = 2012 	if source_d=="Poverty and Inequality Platform (PIP)"	 	& country=="Mali"		 & year==2010 
	...
	****************************** Defining reference series when not the last series
	...
	replace lis=1 if (source_d=="Eurostat Data browser" & country=="Turkiye")
...
*	drop if source==1 & (resource==1 | scale==1) & reverse==1
	...
	* Years with overlapping, ex. Brazil:
	...
	replace nwiids=1   if source_d=="United Nations 1981" & country=="Turkiye"
	replace nwiids=2   if source_d=="Jain 1975"   		  & country=="Turkiye"
	replace overlapf=0 if source_d=="Jain 1975"   		  & country=="Turkiye"
	replace overlapb=0 if source_d=="United Nations 1981" & country=="Turkiye"
	replace overlapf=1 if source_d=="United Nations 1981" & country=="Turkiye"
	
	gen 	zz=(source_d=="United Nations 1981" & country=="Turkiye")
	...
	replace zz=4 if (inlist(source_d, "Leibbrandt et al. 2010", "Leibbrandt et al. 2009", "Whiteford and Van Seventer 2000") | (source_d=="Poverty and Inequality Platform (PIP)" & year<2010) ) & country=="South Africa"
