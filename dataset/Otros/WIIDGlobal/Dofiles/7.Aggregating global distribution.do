	* Code that creates wiidglobal.dta
	...
	**** Bringing population from WIID metadata first
	merge m:m c3 year using "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Data\Metadata\metadata" 			, keepus(c2 c3 population year) update 
	drop _merge
	su population
	
	* Bringing UN Population estimates (x 1,000) for missing countries
	merge m:m c3 year using "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Data\Metadata\population_un.dta" 	, keepus(c3 population year countryun) update 
	drop if c3==""
	drop if inlist(countryun, "American Samoa", "Channel Islands", "Korea, DPR", "New Caledonia", "Northern Mariana Islands")
...
*	tab c3		if country   == "" & year>=1950
	...
	********************* Created with gdp.do
	...
	merge m:m c3 year using "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Data\Metadata\gdp.dta" , keepus(c3 year gdp country* pop*) update
	...
	***************************** 3. Income group and region, updating some missing info (WB)
	...
	cap drop _merge	
	merge m:m c3 using "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Data\Metadata\incomeg", keepus(region_wb incomeg) update
	drop 	if _m==2
	drop _m
	sort c3  country
...
*	lab def incomegroup 5 Missing , add
	...
	* Populating some missing population numbers with WDI, maddison or PWT, make sure they are expressed in same units
	...
	merge m:1 c3 year using "C:\Users\rahul.lahoti\Dropbox\WIID update\WIID 2023\WIID Companion\Data\Metadata\population_wb"
	drop _merge
...
*	edit country year population gdp p50p y_50 gdp if  y_50==.
	...
	** dropping Kosovo and Turkie dur to duplicates 
	bys c3 year: drop if _n==2 & inlist(country,"Turkiye", "Kosovo")
	...
	*	mat ineqac`j' = J(1,1,. )
		...
		***************************** 8. Estimating global fractiles group* ---> countrydistribution
		...
		sort y , stable
		di as result "_______________________________ All"
		qui  gen  sweight = sum(weight)
		qui egen ssweight = sum(weight)
	...
	***************************** 10. Putting together country indices and distribution ----> countrydistribution2a
	...
	forvalues j=$period   {
		...
		*cap erase "countryindices`j'.dta"
		cap drop _m
	}
...
***************************************************************************************************************	
	...
	* Renaming theil to ge
	...
	save "wiidglobal", replace
	export excel using "wiidglobal.xlsx", firstrow(variables) replace	

	global temp former former2 impute newidc cpercentile0 cpercentile00 countrydistribution countrydistribution2a population0 population1 population _gdp0 _gdp1 globaldistribution0 globaldistribution0_wide atkinsonw atkinsonb atkinson atkinsonww ineqindices globaldistribution1_wide globaldistribution2 globaldistribution3 ineqwsc ineqwsreg ineqwsig ineqwsglobal globaldistribution4 shapley globaldistribution5 globaldistribution idwiid 
